...
let tr_function fdef =
  ...
  let get_reg i = if i >= Array.length regs then failwith "out of registers" else regs.(i) in
  ...
  let rec tr_expr i = function
    ...
    | Cst(n)  -> li (get_reg i) n
    | Bool(b) -> if b then li (get_reg i) 1 else li (get_reg i) 0
    ...
    | Var(id) -> begin
        ...
        | Some offset -> lw (get_reg i) offset fp
        ...
        | None -> la (get_reg i) id @@ lw (get_reg i) 0 (get_reg i)
      end
               
    (* Binary operation: use the stack to store intermediate values waiting
       to be used. *)
    | Binop(bop, e1, e2) ->
      let rec number_registers_needed e = 
         match e with
         | Cst(_) -> 1
         | Bool(_) -> 1
         | Var(_) -> 1
         | Binop(_, e1, e2) -> number_registers_needed e1 + number_registers_needed e2
         | Call(_, params) -> List.fold_right (fun e acc -> acc + number_registers_needed e) params 0
      in
       let op = match bop with
         ...
         | Lt  -> slt
       in
       (* If we have 7 registers, we need to push one of them on the stack, because we need 2 registers*)
       if (i = 7) then (
         push (get_reg (i-1))
         @@ tr_expr (i-1) e2
         @@ tr_expr i e1 
         @@ op (get_reg i) (get_reg i) (get_reg (i-1)) 
         @@ pop (get_reg (i-1))

       ) 
      else 
         (
            (* If we need more registers for the first expression, we put the second expression in the last register*)
            if number_registers_needed e1 < number_registers_needed e2 then
               (
                  tr_expr i e2
                  @@ tr_expr (i+1) e1   
               )
    ...
    | Call(f, params) ->
      (* The code generated for the call will use the registers $t0-$t6 to
         pass the arguments. We first save their current values on the stack,
         so that they can be restored after the call. *)
      let code = ref (S "") in
      
      (* Push all the registers from t0 to t(i-1)*)
      for j = 0 to i-1 do
         code := !code @@ push (get_reg j)
      done;
      ...
      let params_code =
      List.fold_right
         (fun e code -> code @@ tr_expr 0 e @@ push t0)
         params nop
      in
      code := !code @@ params_code @@ jal f @@ addi sp sp (4 * List.length params) @@ move (get_reg i) t0;
      (* Pop all the registers from t0 to t(i-1)*)
      for j = i-1 downto 0 do
         code := !code @@ pop (get_reg j)
      done;
      !code
